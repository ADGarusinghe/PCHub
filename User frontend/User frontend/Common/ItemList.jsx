...
const ItemList = () => {
  ...
  const handleSearch = () => {
    ...
    const filteredItems = items.filter(item => {
      ...
      return (filter === 'all' || item.deviceType.toLowerCase() === filter) && matchesSearch;
    });
  ...
  const handleInputChange = (e) => {
    ...
    const filteredItems = items.filter(item => {
      const cleanedTitle =  item.deviceType.toLowerCase().replace(/[^\w\s]/gi, ''); // Remove symbols
  ...
  return (
    <>
      ...
      {loading ? (<div className='container m-4'>
            ...
            <div className="mx-4">
              <select className="form-select" value={filter} onChange={handleFilterChange}>
                ...
                {deviceTypes.map((deviceType, index) => (
                  <option key={index} value={deviceType.deviceType.toLowerCase()}>{deviceType.deviceType}</option>
                ))}
              </select>
            </div>
          </div>
      ...
      </div>): (<div class="spinner-border" style={styles}>
              ...
              <Link to={`/items/${item.id}`} className="text-light text-decoration-none">
                ...
                <div className='row'>
                  <div className='col-12  mr-4'>
                    ...
                    <p className="text-end fs-5">Post added date: {item.date}</p>
                    {item.lastModifyDate && (
                      <p className="text-end fs-6">Last Modification Date: {item.lastModifyDate}</p>
                    )}
                  </div>
                </div>
              </Link>
